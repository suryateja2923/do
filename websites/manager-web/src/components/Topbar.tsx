...
import { Sun, Moon, Bell, Search, Menu, User, LogOut } from 'lucide-react';
...
export const Topbar: React.FC = () => {
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      // Redirect to dashboard or a specific page with search query
    ...
    <header className="sticky top-0 z-30 flex h-16 w-full items-center justify-between border-b border-border bg-card/80 backdrop-blur-md px-6 text-card-foreground">
      ...
      <div className="flex items-center gap-4">
        ...
        <nav className="hidden md:flex items-center gap-2 text-sm font-medium">
          <span className="text-muted-foreground hover:text-foreground transition-colors cursor-pointer" onClick={() => router.push('/')}>
            Manager
          </span>
      ...
      <div className="flex items-center gap-4">
        ...
        <form onSubmit={handleSearchSubmit} className="relative hidden sm:block w-64">
          ...
          <input
            ...
            placeholder="Search owners, properties, complaints..."
            ...
            <div className="absolute right-0 mt-2 w-80 rounded-xl border border-border bg-card p-4 shadow-xl z-50 animate-in fade-in slide-in-from-top-2 duration-200">
              ...
              <div className="space-y-3">
                <div className="flex flex-col gap-1 text-xs border-b border-border/50 pb-2">
                  <span className="font-semibold text-foreground">New Property Registered</span>
                  <span className="text-muted-foreground">Stanza Living Sector 45 is waiting for review.</span>
                  <span className="text-primary/70 text-[10px] mt-1">25 mins ago</span>
                </div>
                <div className="flex flex-col gap-1 text-xs pb-1">
                  <span className="font-semibold text-foreground">Urgent Task Assigned</span>
                  <span className="text-muted-foreground">Confirm water heater issue resolved in Sector 62.</span>
                  <span className="text-primary/70 text-[10px] mt-1">1 hour ago</span>
                </div>
              </div>
            </div>
          )}

...
export const SessionWarning: React.FC = () => {
  ...
  const resetTimers = useCallback(() => {
    ...
    // 1. Inactivity warning timeout (triggered at 14 minutes)
  ...
  // Extend session: refresh tokens and reset the timers
  ...
  useEffect(() => {
    ...
    // Listen to user activities to reset inactivity timers
    const events = ['mousedown', 'keydown', 'scroll', 'touchstart', 'click'];
    const handleActivity = () => {
      // Throttle activity checks slightly
      resetTimers();
    };
      ...
      <div className="w-full max-w-md bg-card border border-border rounded-2xl p-6 shadow-2xl space-y-6 text-center animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            Your administrative session has been idle. You will be logged out automatically in{' '}

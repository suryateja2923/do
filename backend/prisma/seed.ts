...
async function main() {
  ...
  const country = await prisma.country.create({
    data: { id: randomUUID(), name: 'India', code: 'IN' }
  });

  const stateKA = await prisma.state.create({
    data: { id: randomUUID(), country_id: country.id, name: 'Karnataka', code: 'KA' }
  });

  const stateMH = await prisma.state.create({
    data: { id: randomUUID(), country_id: country.id, name: 'Maharashtra', code: 'MH' }
  });

  const cityBLR = await prisma.city.create({
    data: { id: randomUUID(), state_id: stateKA.id, name: 'Bengaluru' }
  });

  const cityMUM = await prisma.city.create({
    data: { id: randomUUID(), state_id: stateMH.id, name: 'Mumbai' }
  });
  ...
  const managerPropertyMappings: any[] = [];
  ...
  const propertyAmenityMappings: any[] = [];
  ...
  for (const prop of fetchedProperties) {
    ...
    for (let r = 1; r <= 5; r++) {
      ...
      rooms.push({
        ...
        room_number: `R-${100 + r}`,
        room_type: r % 2 === 0 ? RoomType.AC : RoomType.NON_AC,
        sharing_type: SharingType.FOUR_SHARING // 4 beds per room capacity
      });
    }
  }
  ...
  for (let i = 0; i < fetchedTenantProfiles.length; i++) {
    ...
    await prisma.bed.update({
      where: { id: bed.id },
      data: {
        occupancy_status: BedOccupancyStatus.OCCUPIED,
        tenant_id: tenant.id
      }
    });
  }
  ...
  const activeBookings = fetchedBookings.filter(b => b.status === BookingStatus.MOVE_IN);
  ...
  const completedBookings = fetchedBookings.filter(b => b.status === BookingStatus.COMPLETED);
  for (let i = 0; i < 2000; i++) {

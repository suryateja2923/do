...
async function testActiveBookingConstraint() {
    ...
    const activeBooking = await prisma.booking.findFirst({
        where: { status: client_1.BookingStatus.MOVE_IN }
    });
    if (!activeBooking) {
...
async function testActiveAgreementConstraint() {
    ...
    const activeAgreement = await prisma.rentalAgreement.findFirst({
        where: { status: client_1.AgreementStatus.ACTIVE }
    });
    if (!activeAgreement) {
    ...
    try {
        ...
        await prisma.rentalAgreement.create({
            data: {
                id: (0, crypto_1.randomUUID)(),
                booking_id: (0, crypto_1.randomUUID)(), // mock random booking id to bypass booking fk check (need to be unique agreement)
...
async function testRLSPolicies() {
    ...
    try {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            await tx.$executeRawUnsafe(`SELECT set_config('request.jwt.claims', $1, true)`, JSON.stringify({ sub: tenantUser.id }));
            const subs = await tx.$queryRawUnsafe(`SELECT id FROM public."OwnerSubscription"`);
        ...
        await prisma.$transaction(async (tx) => {
            await tx.$executeRawUnsafe(`SET LOCAL ROLE authenticated`);
            await tx.$executeRawUnsafe(`SELECT set_config('request.jwt.claims', $1, true)`, JSON.stringify({ sub: ownerUser.id }));
            const subs = await tx.$queryRawUnsafe(`SELECT id FROM public."OwnerSubscription"`);
